...
public class ViewSituatiePanel extends MainPanelAbstract {
	...
	private JTextField textField2c;
	private JTextField textField3c;
	private JTextField textField4c;
	...
	public ViewSituatiePanel(Utilizator utilizator, JLabel statusLbl) {
		...
		JLabel lblCredite = new JLabel("Credite Acumulate");
		lblCredite.setBounds(340, 8, 130, 15);
		add(lblCredite);
		
		textField1c = new JTextField();
		textField1c.setHorizontalAlignment(SwingConstants.CENTER);
		textField1c.setEnabled(false);
		textField1c.setColumns(10);
		textField1c.setBounds(351, 35, 84, 26);
		add(textField1c);
		
		textField2c = new JTextField();
		textField2c.setHorizontalAlignment(SwingConstants.CENTER);
		textField2c.setEnabled(false);
		textField2c.setColumns(10);
		textField2c.setBounds(351, 77, 84, 26);
		add(textField2c);
		
		textField3c = new JTextField();
		textField3c.setHorizontalAlignment(SwingConstants.CENTER);
		textField3c.setEnabled(false);
		textField3c.setColumns(10);
		textField3c.setBounds(351, 115, 84, 26);
		add(textField3c);
		
		textField4c = new JTextField();
		textField4c.setHorizontalAlignment(SwingConstants.CENTER);
		textField4c.setEnabled(false);
		textField4c.setColumns(10);
		textField4c.setBounds(351, 153, 84, 26);
		add(textField4c);
		
		lblTotal = new JLabel("Total:");
		lblTotal.setBounds(200, 196, 48, 15);
		add(lblTotal);
		
		textFieldTa = new JTextField();
		textFieldTa.setHorizontalAlignment(SwingConstants.CENTER);
		textFieldTa.setEnabled(false);
		textFieldTa.setColumns(10);
		textFieldTa.setBounds(255, 191, 84, 26);
		add(textFieldTa);
		
		textFieldTc = new JTextField();
		textFieldTc.setHorizontalAlignment(SwingConstants.CENTER);
		textFieldTc.setEnabled(false);
		textFieldTc.setColumns(10);
		textFieldTc.setBounds(351, 191, 84, 26);
		add(textFieldTc);
		...
		String unprocessedQueryCredit="SELECT sum(d1.puncte_credit)" +
				" FROM optiuni_contract o1, disciplina d1" +
				" WHERE o1.cod_disciplina=d1.cod" +
				" 	AND o1.an_studiu='%d'" +
				"	AND o1.cnp_student='%s'";
		...
		try {
			...
			for(i=1;i<=studyYear;i++)
			{
				...
				//Puncte credit
				query=String.format(unprocessedQueryCredit, i, utilizator.CNP);
				situatie.credite=DatabaseConnection.getSingleValueResult(query);
				
				//Use the data
				if(i==1) {
					textField10.setText(Float.toString(situatie.an)); textField11.setText(Float.toString(situatie.sem1)); textField12.setText(Float.toString(situatie.sem2)); textField1c.setText(Float.toString(situatie.credite));
				}
				
				if(i==2) {
					textField20.setText(Float.toString(situatie.an)); textField21.setText(Float.toString(situatie.sem1)); textField22.setText(Float.toString(situatie.sem2)); textField2c.setText(Float.toString(situatie.credite));
				}
				
				if(i==3) {
					textField30.setText(Float.toString(situatie.an)); textField31.setText(Float.toString(situatie.sem1)); textField32.setText(Float.toString(situatie.sem2)); textField3c.setText(Float.toString(situatie.credite));
				}
				
				if(i==4) {
					textField40.setText(Float.toString(situatie.an)); textField41.setText(Float.toString(situatie.sem1)); textField42.setText(Float.toString(situatie.sem2)); textField4c.setText(Float.toString(situatie.credite));
				}
			}
			for(;i<=4;i++)
			{
			...
			//Total medie
			String query="SELECT AVG(IFNULL(nota,0)) " +
						"FROM optiuni_contract o " +
						"INNER JOIN disciplina d " +
						"	ON d.cod=o.cod_disciplina " +
						"LEFT JOIN catalog c " +
						"	ON o.cod_disciplina=c.cod_disciplina " +
						"WHERE o.cnp_student=\'"+utilizator.CNP+"\'";
			textFieldTa.setText(Float.toString(DatabaseConnection.getSingleValueResult(query)));
			
			//Total credite
			query="SELECT sum(d1.puncte_credit)" +
					" FROM optiuni_contract o1, disciplina d1" +
					" WHERE o1.cod_disciplina=d1.cod" +
					"	AND o1.cnp_student=\'"+utilizator.CNP+"\'";
			textFieldTc.setText(Float.toString(DatabaseConnection.getSingleValueResult(query)));

...
<body>
	...
	<jsp:include page="/include/header.jsp">
		<jsp:param value="Panou de Editare Orar" name="title" />
	</jsp:include>
	...
	<div id="content">
		...
		<%
			ArrayList<OrarComplet> orareT = new OrarCompletWrapper().getOrare("o.id_activitate='"
					+ request.getParameter("id_activitate") + "' AND o.grupa='" + request.getParameter("grupa") + "'");
			OrarComplet orar = orareT.get(0);
			if (request.getParameter("submitted") != null) {
				...
				if (request.getParameter("id_activitate_new").isEmpty() || request.getParameter("grupa_new").isEmpty()
					...
					try {
						...
						System.out.println("Orar existent -> modificata in " + object);
						if (!new OrarWrapper().updateOrar(orar, object)) {
							error("A fost intampinata o problema la introducerea intrarii in baza de date! Aceasta deja exista!",
									out);
						} else
						{
							notify("Orarul " + object + " a fost actualizat. Faceti refresh pentru noile valori in camp.", out);
						}
						orareT = new OrarCompletWrapper().getOrare("o.id_activitate='"
								+ request.getParameter("id_activitate") + "' AND o.grupa='" + request.getParameter("grupa") + "'");
						orar = orareT.get(0);
		...
		<form name="edit_orar" action="edit_orar.jsp?<%=request.getQueryString()%>" method="POST">
			<%
			...
			<table>
				<tr>
					...
					<td>
						...
						%>
						<option value="<%=a.id%>" <%if (orar.idActivitate == a.id) out.write("selected='selected'");%>>
							<%=a.id + " - " + a.denumireDisciplina+" - "+a.numeCadruDidactic%>
						</option>
						<%
							}
						%>
				...
				<tr>
					<td>Grupa:</td>
					<td><input type="text" name="grupa_new" value="<%=orar.grupa%>"></td>
				</tr>
				<tr>
					<td>Sala:</td>
					<td><input type="text" name="sala" value="<%=orar.sala%>"></td>
				</tr>

				<tr>
					<td>Ziua:</td>
					<td><input type="text" name="ziua" value="<%=orar.zi%>"></td>
				</tr>
				<tr>
					<td>Ora:</td>
					<td><input type="text" name="ora" value="<%=orar.ora%>"></td>
				</tr>
				<tr>
					<td>Durata:</td>
					<td><input type="text" name="durata" value="<%=orar.durata%>"></td>
				</tr>
				<tr>
					<td>Frecventa:</td>
					<td><select name="frecventa">
							<option value="<%=Frecventa.Saptamanal%>"
								<%if (orar.frecventa == Frecventa.Saptamanal)
				out.write("selected='selected'");%>>Saptamanal</option>
							<option value="<%=Frecventa.Pare%>" <%if (orar.frecventa == Frecventa.Pare)
				out.write("selected='selected'");%>>Pare</option>
							<option value="<%=Frecventa.Impare%>"
								<%if (orar.frecventa == Frecventa.Impare)
				out.write("selected='selected'");%>>Impare</option>
					</select></td>
				</tr>
